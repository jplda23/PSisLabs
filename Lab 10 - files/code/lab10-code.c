 ...
 * 
 * @param arg indica da thread. É usado para cada thread saber que valores ler do array
...
void * inc_thread(void * arg){
    ...
    pthread_mutex_lock(&mux_myrar);
    while(next_random<LENGTH_ARRAY){
        i=next_random;
        next_random++;
        pthread_mutex_unlock(&mux_myrar);
...
int main(){
    ...
    pthread_mutex_init(&mux_myrar,NULL);
